    ...
    useEffect(() => {
        ...
        function populate(group: THREE.Group) {
            ...
            for (const slab of slabs) {
                ...
                slab.visible = true;
                if (Math.random() > 0.5 && colorMats.length > 0) {
                    const mat = colorMats[(5 * colorIndex) % colorMats.length];
                    if (mat) slab.material = mat;
                    colorIndex++;
                } else if (imageMats.length > 0) {
                    const mat = imageMats[(3 * imageIndex) % imageMats.length];
                    if (mat) slab.material = mat;
                    imageIndex++;
                }
            }
